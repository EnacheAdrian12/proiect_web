...
# def create_tables():
#     connection = sqlite3.connect('blondy.db')
#     connection.execute('''CREATE TABLE User (user_id INTEGER PRIMARY KEY, email TEXT UNIQUE, password TEXT, is_admin BOOLEAN DEFAULT 0)''')
#     connection.execute('''CREATE TABLE Post (post_id INTEGER PRIMARY KEY, title TEXT, content TEXT, user_id INTEGER, FOREIGN KEY (user_id) REFERENCES User(user_id))''')
#     connection.commit()
#     connection.close()
...
def create_connection():
    conn = None
    try:
        conn = sqlite3.connect('blondy.db')
...
def login():
    ...
    if email is None or password is None:
        return "Email-ul sau parola sunt invalide", 400
...
def register():
    ...
    if connection.execute("SELECT * FROM User WHERE email = ?", (email,)).fetchone() is not None:
        return "User-ul exista deja",  400
...
def create_post():
    ...
    if title is None or content is None:
        return "Introduceti date corecte", 400
...
def update_email_route():
    ...
    email = request.json.get('email', None)
    if email is None:
        return "Introduceti un email valid", 400
